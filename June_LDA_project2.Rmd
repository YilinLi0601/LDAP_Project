...
colnames(desc_by_wave) <- gsub("malaise", "Post COVID-19 Symptoms", colnames(desc_by_wave))
...
for(w in 1:3) {
  ...
  rownames(cor_matrix) <- colnames(cor_matrix) <- c(
    "General Health Questionnaire", "Post COVID-19 Symptoms", 
...
covid_comparison <- clean %>%
  ...
  summarise(
    ...
    `Post COVID-19 Symptoms` = paste0(round(mean(malaise, na.rm = TRUE), 2), " (", 
                                     round(sd(malaise, na.rm = TRUE), 2), ")"),
...
var_labels <- c("General Health Questionnaire", "Post COVID-19 Symptoms", 
                "Generalised Anxiety Disorder", "Loneliness Scale")
...
cat("\n=== ANALYSIS COMPLETE ===\n")
```
# Hypothesis 3: Lonliness is a mediator between the relationship of COVID positivity and mental health (general anxiety & malaise).
...
# Path diagram
```{r}
cat("The DAG diagram showing relationships between variables")
dag <- dagitty("dag {
...
plot(dag)
cat("The SEM model-based path diagram.")
